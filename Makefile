...
all: $(BINDIR)/vm 
apps: $(BINDIR)/hello.so $(BINDIR)/sleep.so $(BINDIR)/file.so $(BINDIR)/thread.so $(BINDIR)/mutex.so $(BINDIR)/memory.so $(BINDIR)/shell.so $(BINDIR)/haha.so
...
$(APPDIR)/%.o : $(APPDIR)/%.cpp 
	$(CXX) -c  -g $(APPCFLAGS) $(CPPFLAGS) $< -o $@
	
$(OBJDIR)/%.o : %.c
	$(CC) -c  -g $(CFLAGS) $< -o $@

$(OBJDIR)/%.o : %.cpp 
	$(CXX) -c  -g $(CFLAGS) $(CPPFLAGS) $< -o $@
